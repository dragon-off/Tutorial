		...
		obj_split = line_split[r].split('^');
					...
					if(obj_split[0]=='es') {
						error_state = obj_split[1]*1;
							if(error_state==1 && customer_present==1 && counter_item_button==(143)) { //проверка в перерыве или нет станция
				...
				//		document.getElementById(obj_split[0]).innerHTML = obj_split[1];
					...
					if(obj_split[0]=='msg_top_nc') {
						...
						for(var i=0;i<=(a.length-1);i++) {
							...
							for(var j=0;j<=(b.length-2);j++) {
								if(b[j].length>0) {
								//alert(b[j]);
									objSel.options[j] = new Option(b[j].split("|")[1], b[j].split("|")[0]*1);
								};
							};
						}
					...
					if(obj_split[0]=='blink' & blink_a==0) {
						if(counter_item_button!=(143) && counter_item_button!=(145)) {
							type_avalible(2);
							disable_3=true;
							blink_a=array_500*2;
							blink_b=0;
...
function run_qwin(command,value) 
{
 ...
 sbrf.log("Запрос "+command);
	...
	//	value=value.substr(3,4);
		httpRequest('/pult?pultfl=1&type='+command+'&workstation='+ws+'&value='+value*1+'&type1=update', function(req) { update_split(req.responseText) }, function(req) { document.getElementById('msg_timeout').style.display='block';}, null, 20000);
		...
		httpRequest('/pult?pultfl=1&type='+command+'&workstation='+ws+'&value='+value, function(req) { update_split(req.responseText) }, function(req) {  httpRequest('/login/login.action?username=Administrator', function(req) { }, function(req) { }, null, 20000); document.getElementById('msg_timeout').style.display='block';}, null, 20000);
		...
		httpRequest('/pult?pultfl=1&type='+command+'&workstation='+ws+'&value='+value+'&type1=update', function(req) { update_split(req.responseText) }, function(req) {  httpRequest('/login/login.action?username=Administrator', function(req) { }, function(req) { }, null, 20000); document.getElementById('msg_timeout').style.display='block';}, null, 20000);
...
//		httpRequest('run_call_number_from_queue.qsp2?type="'+command+'"&workstation='+ws+'&value='+value, function(req) { update_split(req.responseText) }, function(req) {  httpRequest('/login/login.action?username=Administrator', function(req) { }, function(req) { }, null, 20000); document.getElementById('msg_timeout').style.display='block';}, null, 20000);
		...
		httpRequest('/pult?pultfl=1&type='+command+'&workstation='+ws+'&value='+value+'&type1=update', function(req) { update_split(req.responseText);  
		//httpRequest('run_perevod.qsp2?type="'+command+'"&workstation='+ws+'&value='+value, function(req) { update_split(req.responseText);  
		clearTimeout(auto_nextTimer); auto_nextTimer=setTimeout(function() { run_qwin('next',1); },auto_next); }, function(req) {  httpRequest('/login/login.action?username=Administrator', function(req) { }, function(req) { }, null, 20000); document.getElementById('msg_timeout').style.display='block';}, null, 20000);
		...
		httpRequest('/pult?pultfl=1&type='+command+'&workstation='+ws+'&logincod='+logincod+'&value='+value, function(req) { update_split(req.responseText) }, function(req) {  httpRequest('/login/login.action?username=Administrator', function(req) { }, function(req) { }, null, 20000); document.getElementById('msg_timeout').style.display='block';}, null, 20000);	
