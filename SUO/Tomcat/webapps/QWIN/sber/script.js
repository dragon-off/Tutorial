...
*/
 ...
 sbrf.log("Смена режима отображения кнопок="+type);
	...
	if(type==3) //Перерыв, повиший вызов
		{
		change_avalible('in_work',0);
		if((counter_item_button==(183) && counter_item_button==(185)) & customer_present==1) {
	...
	if(el!='next' & el!='in_work') 
	{
		...
		{ 
			...
			if(el=="pereriv") {  blink_a=0;
				...
				if(customer_present==1 && counter_item_button==(183)) { //проверка в перерыве или нет станция
					run_qwin('pereriv',1);
					type_avalible(0);
				} else {
		...
		if(obj.style.backgroundImage==image_4 || obj.style.backgroundImage==image2_4) 
			{//alert(el);
				if(el=="next") { 
					...
					run_qwin('next',999);wait_next();
				};	
				if(el=="in_work") {  blink_a=0;
			
					document.getElementById("next").value="Следующий";
					if(customer_present==1 && counter_item_button==(185)) { //проверка в внутреней работе или нет станция
						run_qwin('in_work',1);
						type_avalible(0);
					} else {
						run_qwin('in_work',0);
