...
setTimeout("wtimerefresh()",600000);
...
httpRequest(http_url, function(req) { loadcardlist2(req.responseText) }, function(req) { loadcardlist()}, null, 3000);
}
...
{   
	...
	//alert(button);
	http_url="print.qsp2?printer="+printer+"&button="+button;
	httpRequest(http_url, function(req) { resultat(req.responseText) }, null, null, 3000);
	plusbutton=0;
};		
...
function redtaktika(button,value)
{
	if(plusbutton<41) {
		if(value=="none") {
			...
			httpRequest(http_url, function(req) { redtaktika(button,req.responseText) }, function(req) { redtaktika(button,"none")}, null, 3000);
		} else {
