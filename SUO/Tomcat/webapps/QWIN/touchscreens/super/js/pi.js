...
function load_pi(ticket_print) {
	if(ticket_print>0)
	{
	httpRequest('/get?loadqueue', function(req) {if(req.responseText.search("Please wait")==-1) { load_queue(req.responseText); run_prognoz();if(ticket_print>1)ticket_print =1; print_result(ticket_print);} else startsession();}, function(req) {if(ticket_print>0 && ticket_print<3) { ticket_print++; setTimeout(function() {load_pi(ticket_print);},3000);} else {if(ticket_print>0) startsession(); else setTimeout(function() {load_pi();},10000); }}, null, 20000);
	}
	else
	{
...
event_122="STATISTIC "+sec_to_time(TIME_NO*2)+" TIME ";
	...
	httpRequest(set_jsp, function(req) {console.log("Прогноз отправлен!");setTimeout(function() {load_pi();},10000);}, function() {console.log("Не выполнился set_pvo.jsp!",run_arr); setTimeout(function() {load_pi();},10000);}, null, 3000);	
...
function show_red(r_butt,rrred)
{
	...
	//var run_arr="/get?runprognoz=1&print_time="+inttom(ticket_print_time*1)+"&print_chel="+ticket_print_chel+"&printer="+p;
