...
function load_pi(ticket_print) {
	httpRequest('/touchscreens/super/load_queue.qsp2', function(req) { load_queue(req.responseText); run_prognoz(); print_result(ticket_print);}, function(req) {  }, null, 20000);
	return true;
};
...
if(!button_run[i])
	...
	//	console.log(queue[j]["ITEM_TRACK"]);
		if(((queue[j][sar("BUTTON")] == 80+ws*1) ||(queue[j][sar("ITEM_BUTTON")] == i.toString())) && //проверка что это искомая кнопка или переведнный клиент в окно
...
function del_ticket(ticket,ws) {
	if(ticket) {
		if(ticket["LAST"]==true) {
			...
			log_debug("НАЙДЕНО прог. вр. ожид. опер. №"+ticket[sar("ITEM_BUTTON")]+"   клиентов:"+counter[ws][sar("QUEUE")]+"  рас. вр. ожид.:"+counter[ws][sar("TT")]+" приор. клиента:"+l_name[ticket[sar("LEVEL")]]);
...
var ws=[];
	...
	(next_level["1"][1]==next_level["2"][1] ))) {
		log_debug("вызов в окно "+ws[sar("COUNTER")]+" по приор. в окно 1 "+next_level["1"][1]+" опер. "+next_level["1"][2][sar("ITEM_BUTTON")]);
		del_ticket(next_level["1"][2],ws_с);	
		return i;
	} else if(next_level["2"][2]) {
			log_debug("вызов в окно "+ws[sar("COUNTER")]+" по приор. в окне 2 "+next_level["2"][1]+" опер. "+next_level["2"][2][sar("ITEM_BUTTON")]);
