...
//update('data.qsp2?button='+ticket,'buttons');
update('/get?monitoring','buttons');
update('/settings_admin/day.jsp','day');

setTimeout("start_info_buttons()",20000);
//setTimeout(function() { start_prognoz(); },1000);	
//setTimeout(function() { start_prognoz(); },31000);		
};
...
for(var i=1;i<=3;i=i+2) //{fix}
	...
	if(precent>0)
		{
		precent = precent+'%';
		td[i-1].style.display = 'block';
		}
	else
		{
		if(parent!='svetofor_place')
...
imag.setAttribute('src', '../images/big_'+str[2]+'.jpg');
...
for(var i=1;i<=len;i++)
	...
	var row = str[i].split('|');
	var td=new Array();
	if(row[1]=="-") tr[i-1].style.display = 'none';
	for(var j=1;j<=9;j++)
		{
		...
		if(j!=9)
			...
			var prevel = document.getElementById('buttons-td-'+((i-1)*9+j));
			var resprevel = '';
			if(prevel!=undefined && prevel!=null) resprevel= prevel.innerHTML;
			if(row[j]*1==1 && resprevel!='')
				{
				imag.setAttribute('src', '../images/yes.png');
				}
			else
				{
...
for(var i=1;i<=len;i++)
	...
	for(var j=0;j<11;j++)
		...
		if(j==0)
			{
			var buf_row0 = row[0].split('~');
			var ul = 0;

			if(buf_row0.length>1)
				{
				row[0] = buf_row0[0];
				ul = buf_row0[1]*1;
				}
		...
		/*if(j==9)
			...
			td[j].appendChild(imag);
				...
				if(j7.length>1)
					...
					if(j7[0]=='r')
					{
					imag.setAttribute('src', '../images/red.png');
					}
				else
					{
					if(j7[0]=='g')
						{
						imag.setAttribute('src', '../images/green.png');
						}
					else
						imag.setAttribute('src', '../images/yellow.png');
					}
					imag.setAttribute('width', '20');
					imag.setAttribute('height', '20');
...
if(str[5]!='\n')
	...
	if(smess5_buf!=str[5] || counter_marker == 0)	
		{
		...
		if(len>1)
			...
			setTimeout("counter_marker =0;console.log('!!!counter_marker = 0');",3000);	
			}
		}
	}
if(str[3]!='\n')
	{
	if( counter_marker == 0 || smess2_buf!=str[3])	
		{
		if(smess2_buf!=str[3]) console.log('smess2_buf!=str[3]');
		if(counter_marker == 0) console.log('!2!counter_marker = 0');
		...
		if(len>1)
			...
			timeouts[3]=setTimeout("counter_marker =0;console.log('!!!counter_marker = 0');",3000);	
			}
		}
	}
