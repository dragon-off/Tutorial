...
//takt dlja UL
operation_takt[181]=[];
operation_takt[181]["id"]=181;
operation_takt[181]["takt"]=300;
operation_takt[181]["rwt"]=0;
...
function startsession(){
	window.location.href= "http://localhost/login/login.action?username=administrator&page=/touchscreens/prognoz/index.qsp2/"
}
...
function load_pi(ticket_print) {
	if(p==1 || ticket_print>0)
	{
	//httpRequest('/get_load_queue', 
	
	httpRequest('/get?loadqueue',
	function(req) {
	if(req.responseText.search("Please wait")==-1)
		{ 
		
		load_queue(req.responseText);
		
		run_prognoz();
		if(ticket_print>1)
			ticket_print =1; 
		print_result(ticket_print);
		}
		else 
		startsession();
		}, 
	function(req) {
	if(ticket_print>0 && ticket_print<3)
		{
		ticket_print++;
		setTimeout(function() {load_pi(ticket_print);},350000);
		} 
	else 
		{
		if(ticket_print>0)
			...
			setTimeout(function() {load_pi();},350000);
		}
	},
	null,
	20000);
	...
	{
///	console.log('Рассчет прогноза не производим, принтер =',p);
	}
	return true;
};
...
function print_result(ticket_print) {
if(result_load_pi==true)
{
if(ticket_print==1)
	{
	var buf_but = red_button;
	var buf_level = 7;
	if(buf_but>40)
		{
		buf_but = buf_but-40;
	...
	//console.log(buf_level,buf_but);
	if(prognoz_queue[buf_level]!=undefined && prognoz_queue[buf_level][buf_but]!=undefined && prognoz_queue[buf_level][buf_but]!=null)
		{
		ticket_print_time = prognoz_wt[buf_level][buf_but]*1+60;
		ticket_print_chel = prognoz_queue[buf_level][buf_but]*1;
		prognoz_queue[buf_level][buf_but] = null;
		}
	else
		{
		ticket_print_time = 0;
	...
	show_red(red_button,ticket_print);
	}
else
	{
	...
	prognoz_to_qwin(ticket_print);
	}
for (var r=1;r<=7;r++)
	{
	for (var e=1;e<=20;e++)
		{
		if(prognoz_queue[r][e]!=undefined)
			{
			prognoz_wt[r][e]=null;
			prognoz_queue[r][e]=null;
...
event_122="STATISTIC "+sec_to_time(TIME_NO*2)+" TIME ";
	...
	if(stop_122==false)
		httpRequest("/get?pvo=\""+event_122+"\"", function(req) {}, function() {}, null, 3000);
		//httpRequest("send_122.jsp?pvo=\""+event_122+"\"", function(req) {}, function() {}, null, 3000);
	stop122=true;
	httpRequest(set_jsp, function(req) {console.log("Прогноз отправлен!");document.getElementById("body").innerHTML = "1";setTimeout(function() {load_pi();},50000);}, function() {console.log("Не выполнился set_pvo.jsp!",set_jsp);document.getElementById("body").innerHTML = set_jsp; setTimeout(function() {load_pi();},50000);}, null, 3000);	
	////httpRequest(run_arr, function(req) {setTimeout(function() {load_pi();},50000);}, function() {console.log("Не выполнился runPrognoz.qsp2!",run_arr); setTimeout(function() {load_pi();},50000);}, null, 3000);
