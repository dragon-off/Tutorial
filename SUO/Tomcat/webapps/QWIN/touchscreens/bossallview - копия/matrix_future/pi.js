...
function load_pi(ticket_print) {
	httpRequest('/get?loadqueue', function(req) { load_queue(req.responseText);  run_prognoz(); print_result(ticket_print);}, function(req) {  }, null, 3000);
	return true;
};
...
if(counter[ws][35]==0) {
	if(but_cache_vip[i]!=null && "1"==vip.toString() ) { //&& queue[but_cache_vip[i]][sar("LEVEL")]!=2
	////	log_debug( "226");
	
		return queue[but_cache_vip[i]];
	}
	if(but_cache[i]!=null && "0"==vip.toString()) {
	////	log_debug("230 "+queue[but_cache[i]]);
...
//	if(queue[j][sar("BUTTON")] == 80+ws*1) {
//		log_debug("   80+ws*1	true");
//	} else {
//		log_debug("   80+ws*1	false");
//	};
if(ws==1)
	if(queue[j][sar("ITEM_BUTTON")] == i.toString() &&
		(!queue[j][sar("BUTTON")] || queue[j][sar("BUTTON")]<80)) {
		log_debug("   !2	true "+queue[j][sar("LEVEL")]+"  b="+queue[j][sar("BUTTON")]+" i="+i);
	} else {
		log_debug("   !2	false "+queue[j][sar("LEVEL")]+"  b="+queue[j][sar("BUTTON")]+" i="+i);
	};
/*	if((max_if>=max  && level==queue[j][sar("LEVEL")]) || (level>queue[j][sar("LEVEL")])) {
		log_debug("   !3	true");
	} else {
		log_debug("   !3	false");
	};
	if((queue[j][sar("LEVEL")]!=1 && queue[j][sar("LEVEL")]!=2) || "1"==vip.toString()) {
		log_debug("   !4	true");
	} else {
		log_debug("   !4	false");
	};*/
		if(((queue[j][sar("BUTTON")] == 80+ws*1) || (queue[j][sar("ITEM_BUTTON")] == i.toString() &&
		(!queue[j][sar("BUTTON")] || queue[j][sar("BUTTON")]*1<80))) && 
			((max_if>=max  && level==queue[j][sar("LEVEL")]) || (level>queue[j][sar("LEVEL")])) && 
			((queue[j][sar("LEVEL")]!=1 && queue[j][sar("LEVEL")]!=2) || "1"==vip.toString())) {
				log_debug("   254	"+queue[j][sar("LEVEL")]);
				level=queue[j][sar("LEVEL")];
				max=max_if;
				ticket=queue[j][sar("ITEM_TRACK")];//j;
		};
	};
	if(ticket!=null) {
...
if( !ticket["LAST"])
	log_debug("ws = "+ws+"     ticket ITEM_BUTTON="+ticket[sar("ITEM_BUTTON")]+" BUTTON="+ticket[sar("BUTTON")]+" LEVEL="+ticket[sar("LEVEL")]+"  counter_QUEUE"+counter[ws][sar("QUEUE")]);
if( ticket["LAST"] && ticket[sar("LEVEL")]==2)
	log_debug("last ws = "+ws+"     ticket ITEM_BUTTON="+ticket[sar("ITEM_BUTTON")]+" BUTTON="+ticket[sar("BUTTON")]+" LEVEL="+ticket[sar("LEVEL")]+"  counter_QUEUE"+counter[ws][sar("QUEUE")]);
	if(ticket) {
	//console.log(ticket);
		if(ticket["LAST"]==true) {
			prognoz_wt[ticket[sar("LEVEL")]][ticket[sar("ITEM_BUTTON")]]=counter[ws][sar("TT")];
		////	if(ticket[sar("LEVEL")] <3)
		////		log_debug("last ITEM_BUTTON "+ticket[sar("ITEM_BUTTON")]+" level "+ticket[sar("LEVEL")]+": "+counter[ws][sar("QUEUE")]+" ws="+ws+" item_s="+ticket[sar("ITEM_START")]+" item_t="+ticket[sar("ITEM_TRACK")]);
			//log_debug("(LAST) Талон "+ticket[sar("LEVEL")]+" "+ticket[sar("ITEM_BUTTON")]+" С прогнозной очередью "+prognoz_queue[ticket[sar("LEVEL")]][ticket[sar("ITEM_BUTTON")]]
			//+" пойдет в окно "+ws);
	...
	////		if(ticket[sar("LEVEL")] < 3)
	////			log_debug("vip пойдет в окно "+ws+" кол."+counter[ws][sar("QUEUE")]+"   ITEM_BUTTON="+ticket[sar("ITEM_BUTTON")]+" item_s="+ticket[sar("ITEM_START")]+" item_t="+ticket[sar("ITEM_TRACK")]);
			counter[ws][sar("QUEUE")]++;
			//	log_debug(" Талон "+ticket[sar("LEVEL")]+" "+ticket[sar("ITEM_BUTTON")]+" С прогнозной очередью "+prognoz_queue[ticket[sar("LEVEL")]][ticket[sar("ITEM_BUTTON")]]
			//	+" пойдет в окно "+ws);
		////	log_debug("===="+(operation_takt[(ticket[sar("ITEM_BUTTON")]*1)]["takt"]*1)+"   ws="+ws+"    tick="+ticket[sar("ITEM_BUTTON")]+" item_s="+ticket[sar("ITEM_START")]);
...
log_debug("ищим клиента для окна:"+ws_с);
var ws=[];
	ws=counter[ws_с];
	...
	for(var j in ws) {
		...
		if(j>0 && j<21) {
			...
			if(ticket) {
				log_debug("		S"+j+"	"+ticket[sar("ITEM_TRACK")]+" l="+ticket[sar("LEVEL")]+"   "+ticket);
				max_if=(TIME_NO-ticket[sar("ITEM_START")]*1);
				if((ticket[sar("LEVEL")]<next_level[ws[j]][1]) ||
				((ticket[sar("LEVEL")]==next_level[ws[j]][1]) && next_level[ws[j]][0]<=max_if)) {
					
					log_debug("			true||||||||||||  "+ticket[sar("LEVEL")]+"<"+next_level[ws[j]][1]+" "+next_level[ws[j]][0]+"<="+max_if+"   "+ticket[sar("ITEM_START")]+" "+TIME_NO);
					
					next_level[ws[j]][2]=ticket;
					next_level[ws[j]][1]=ticket[sar("LEVEL")];
					next_level[ws[j]][0]=max_if;
					log_debug("			1true||||||||||||  "+ticket[sar("LEVEL")]+"<"+next_level[ws[j]][1]+" "+next_level[ws[j]][0]+"<="+max_if+"   "+ticket[sar("ITEM_START")]+" "+TIME_NO);
	...
	(next_level["1"][1]==next_level["2"][1] ))) {
		log_debug("1 бфле решено вызвать:"+next_level["1"][2]);
		del_ticket(next_level["1"][2],ws_с);	
		return i;
	} else if(next_level["2"][2]) {
		log_debug("2 бфле решено вызвать:"+next_level["2"][2]);
...
for(var e=1;e<=20;e++)
	...
	if(document.getElementById(this_id))
		{
		document.getElementById(this_id).innerHTML = array_clients[e-1];//+" "+((prognoz_queue[2][e]!=undefined && prognoz_queue[2][e]!=null)?prognoz_queue[2][e]:0);
		}
