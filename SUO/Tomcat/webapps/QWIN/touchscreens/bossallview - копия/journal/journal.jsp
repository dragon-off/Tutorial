...
sql="UPDATE Journal SET comment = 'Вход/Выход пользователя в систему СУО '+(select UserName from Journal x where x.id=Journal.id) WHERE  comment like'В¬С…РѕРґ РїРѕР»СЊР·РѕРІР°С‚РµР»В¤:%'; UPDATE Journal SET comment = 'Вход/Выход пользователя в систему был произведен с ошибкой'+(select UserName from Journal x where x.id=Journal.id) WHERE  comment like'В¬С…РѕРґ Р°РґРјРёРЅРёСЃС‚СЂР°С‚РѕСЂР° СЃРјРµРЅС‹%'"+sql;
if(wh>0)
	{
	int isfirst=0;
	sql+= " where ( ";
